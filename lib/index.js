...
$(document).ready(() => {
  ...
  $("button").on("click", () => {
    ...
    getAllColors($('.colorized-text').val()).forEach(color => postColor(color))


    getUniqueColors($("textarea").val()).forEach(color => {$(".colorized-text").append(buildSwatch(color))})
  })

  $('textarea').keypress((e) => {
   if(e.which === 13){
     e.preventDefault()
     console.log("You pressed the enter key, will generate swatches")
   }
  })
})
...
const getAllColors = (string) => {
  ...
  return allWords.filter(word => COLORS[word])
}

const getUniqueColors = (string) => {
  getAllColors(string)
  return [...new Set(getAllColors(string))]
}
